...
import {getUserDocument} from '../handlers/read'
...
module.exports = {
    ...
    run: function (bot, raw) {
        ...
        if (member) {
            ...
            getUserDocument(member.id).then((doc) => {
                if (doc) {
                    obj.changed += `\n► Previous stored names: \`\`\`xl\n${doc.names ? doc.names.filter((name, pos) => doc.names.indexOf(name) === pos).join(', ').substr(0, 1200) : 'None'}\`\`\``
                } else {
